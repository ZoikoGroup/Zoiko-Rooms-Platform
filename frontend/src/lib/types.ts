...
export interface Listing {
  ...
  // moved in. Computed server-side (crud.listing.is_listing_available) --
...
export interface Occupancy {
  ...
  moveOutRequestedAt: string | null;
  createdAt: string;
  endedAt: string | null;
}
...
export interface SimulatedPayment {
  ...
  // Resolved server-side from the occupancy behind this payment's first
  // allocation, if any -- empty/null when a payment has no allocation yet.
  guestName: string;
...
export interface UserApplication {
   ...
   *  identically for an approval and a rejection. Null while still
   *  SUBMITTED/WITHDRAWN. */
