...
def get_public_listing(
    ...
    Exception: a signed-in USER who has ever applied to this specific listing
    can still open it even once it's no longer "available" (occupied, since
    they're now the occupant, or paused) -- "hidden from new applicants" must
    not also mean "you can't see the room you're already renting." Applies to
    any application status, not just an accepted one, so a rejected applicant
    can still review what they applied for."""
                ...
                has_applied = (
                    db.scalar(
                        select(Application.id).where(
                            Application.listing_id == listing_id, Application.guest_id == guest.id
                        )
                    )
                    is not None
                )
